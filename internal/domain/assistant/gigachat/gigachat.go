...
const (
	...
	promptAudio         = `Сделай краткую выжимку из расшифровки совещания: суть обсуждения, ключевые решения, договоренности, задачи, ответственные и сроки. Коротко, структурированно, без воды, ничего не додумывай. Из этого текста `
	promptVoice         = `Сделай краткую выжимку из расшифровки голосового сообщения: суть, важные факты, просьбы/поручения, следующие действия. Коротко, без воды, ничего не додумывай. `
)
...
func NewGigaChatClient(ctx context.Context, cfg *config.Config, logger *zap.Logger) (*GigaChatClient, error) {
	gigaChat := &GigaChatClient{
		client:   resty.New(),
		authHost: cfg.GigaChat.Auth,
		authKey:  cfg.GigaChat.Token,
		mainHost: cfg.GigaChat.Main,
		lgr:      logger,
	}
